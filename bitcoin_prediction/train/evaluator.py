...
def evaluate_model(
    ...
    for i, feature in enumerate(feature_columns):
        ...
        logger.info(
            f"Metrics for {feature}: MSE={mse:.6f}, MAE={mae:.6f}, RMSE={np.sqrt(mse):.6f}"
        )
...
def make_predictions(
    ...
    for _ in range(num_predictions):
        ...
        tensor_input = torch.tensor(
            current_sequence.reshape(1, sequence_length, -1), dtype=torch.float32
        ).to(device)
    ...
    future_dates = pd.date_range(
        start=data.index[-1] + pd.Timedelta(days=1), periods=num_predictions, freq="D"
    )
